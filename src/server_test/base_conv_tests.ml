...
let encode_decode_tests =
    ...
    in
    let check (n, b) =
        let encoded = base_of_int n b in
        let decoded = int_of_base encoded b in
...
let base_list_tests =
    let open CCList.Infix in
    let cases = 2 -- 6 >>= fun base ->
        2 -- 4 >>= fun size ->
        let m = CCInt.pow base size in
        2 -- m >|= fun n -> (n, base, size)
    in
    let check (n, base, size) =
        let encoded = base_list_of_int ~base ~size n in
        let decoded = int_of_base_list ~base encoded in
        ae ~printer:string_of_int n decoded
    in
    cases_of check cases
...
let suite = [
    ...
    "base_list_tests" >::: base_list_tests;
]
