...

{
  ...
  describe("ClientMessages", () => {
    ...
    test("Client_theme_classic", () =>
      test_convert(
        make_client_theme("some other token xxx777", Theme.Classic),
      )
    );

    test("Client_theme_open_source", () =>
      test_convert(
        make_client_theme("some other token yyy888", Theme.Open_source),
      )
    );
  });
};
